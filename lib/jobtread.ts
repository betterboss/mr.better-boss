...
export class JobTreadClient {
  ...
  private async query(graphqlQuery: string, variables?: Record<string, unknown>) {
    ...
    if (data.errors) {
      throw new Error(`JobTread GraphQL error: ${data.errors[0]?.message || 'Unknown error'}`);
    }
  ...
  // Demo data for when API keys aren't configured or for demo mode
  ...
  private getMockSchedule(): ScheduleEvent[] {
  ...
  private getMockFinancials(): FinancialSummary {
    return {
      totalRevenue: 187500,
      totalCosts: 118200,
      grossProfit: 69300,
